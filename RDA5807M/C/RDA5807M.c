...
void I2C_Start(void)
{
    RDA6807M_Write_SDA(GPIO_PIN_SET);   //需在SCL之前设定
    RDA6807M_Write_SCL(GPIO_PIN_SET);   //硬件进入SDA检测状态
    Delay();                            //延时至少4.7us
    RDA6807M_Write_SDA(GPIO_PIN_RESET); // SDA由1->0,产生开始信号
    Delay();                            //延时至少4us
    RDA6807M_Write_SCL(GPIO_PIN_RESET); // SCL变为无效
}

//产生STOP信号
void I2C_End(void)
{
    RDA6807M_Write_SDA(GPIO_PIN_RESET); //在SCL之前拉低
    RDA6807M_Write_SCL(GPIO_PIN_SET);   //硬件进入SDA检测状态
    Delay();                            //至少延时4us
    RDA6807M_Write_SDA(GPIO_PIN_SET);   // SDA由0->1,产生结束信号
    Delay();                            //延时至少4.7us
}
//由主向从发送应答码
// 0-ACK,1-NAK
...
{
    if (ack == 1)
        RDA6807M_Write_SDA(GPIO_PIN_SET); //产生应答电平
    else
        RDA6807M_Write_SDA(GPIO_PIN_RESET);
    Delay();
    RDA6807M_Write_SCL(GPIO_PIN_SET);   //发送应答信号
    Delay();                            //延时至少4us
    RDA6807M_Write_SCL(GPIO_PIN_RESET); //整个期间保持应答信号
}
...
{
    uint8_t ret;                      //用来接收返回值
    RDA6807M_Write_SDA(GPIO_PIN_SET); //电阻上拉,进入读
    Delay();
    RDA6807M_Write_SCL(GPIO_PIN_SET); //进入应答检测
    Delay();                          //至少延时4us
    ret = RDA6807M_Read_SDA();        //保存应答信号
    RDA6807M_Write_SCL(GPIO_PIN_RESET);
    return ret;
}
...
uint8_t I2C_SendByte(uint8_t dat)
{
    ...
    while (loop--)
    {
        // 高在前低在后
        if (dat & 0x80)
            RDA6807M_Write_SDA(GPIO_PIN_SET);
        else
            RDA6807M_Write_SDA(GPIO_PIN_RESET);
        Delay();
        RDA6807M_Write_SCL(GPIO_PIN_SET);
        Delay(); //延时至少4us
        RDA6807M_Write_SCL(GPIO_PIN_RESET);
        dat <<= 1; //低位向高位移动
    }
